...
if (!function_exists('vuecli')) {
     ...
     */
    function vuecli($bundle, $ignore = true)
    {
        static $assets = null;
        $distPath = config('vuecli.dist_path', 'admin');
        $useVuePath = config('vuecli.use_path', true);
        $ssl = config('vuecli.dev_server.https', false);
        $server = config('vuecli.dev_server.ip', 'localhost');
